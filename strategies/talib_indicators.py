...
def simulate_strategy(strategy, ticker, current_price, historical_data, account_cash, portfolio_qty, total_portfolio_value):
   ...
   print(f"{historical_data[historical_data['Close'].isnull()] = }")
   print(f"{ticker = }, {total_portfolio_value = }, {trade_asset_limit = }, {max_investment = }, {current_price = }, {account_cash = }, {portfolio_qty = }, {action = }")
   if action == 'Buy':
