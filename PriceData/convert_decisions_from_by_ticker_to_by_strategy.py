...
def convert_decisions_from_by_ticker_to_by_strategy_new(
    con_source, con_target, strategies, logger
):
    """
    Convert ticker-based strategy decisions to strategy-based tables.
    ...
    # Create a mapping of strategy functions to their signal column names
    strategy_signal_mapping = {}

    # First, get a sample ticker to identify all available columns
    if tickers:
        sample_ticker = tickers[0]
        sample_df = pd.read_sql(f"SELECT * FROM '{sample_ticker}'", con_source)
        all_columns = set(sample_df.columns)

        logger.info(f"Available columns in database: {all_columns}")

        # For each strategy, find its corresponding signal columns
        for strategy in strategies:
            original_strategy_name = strategy.__name__

            # Remove common suffixes from strategy name for matching
            strategy_name = original_strategy_name
            for suffix in ["_indicator"]:
                # for suffix in ["_indicator", "_strategy", "_algo"]:
                if strategy_name.endswith(suffix):
                    strategy_name = strategy_name[: -len(suffix)]
                    break

            logger.info(
                f"Looking for columns for strategy: {original_strategy_name} (base name: {strategy_name})"
            )

            # Common naming patterns for strategy signals
            possible_patterns = [
                f"{strategy_name}_Signal",
                # f"{strategy_name}_signal",
                # f"{original_strategy_name}_signal",
                # f"{original_strategy_name}_Signal",
                # f"{strategy_name}_buy_sell",
                # f"{original_strategy_name}_buy_sell",
                # f"{strategy_name}_decision",
                # f"{original_strategy_name}_decision",
                # strategy_name,
                # original_strategy_name,
            ]

            # Find columns that match any of the patterns
            matching_columns = []
            for pattern in possible_patterns:
                exact_matches = [col for col in all_columns if col == pattern]
                prefix_matches = [
                    col for col in all_columns if col.startswith(f"{pattern}_")
                ]
                matching_columns.extend(exact_matches)
                matching_columns.extend(prefix_matches)

            # If no direct matches, look for any column containing the strategy name
            if not matching_columns:
                matching_columns = [
                    col
                    for col in all_columns
                    if strategy_name.lower() in col.lower()
                    or original_strategy_name.lower() in col.lower()
                ]

            if matching_columns:
                strategy_signal_mapping[original_strategy_name] = matching_columns
                logger.info(
                    f"Found signal columns for {original_strategy_name}: {matching_columns}"
                )
            else:
                logger.warning(
                    f"No signal columns found for strategy: {original_strategy_name}"
                )
    ...
    for ticker in tickers:
        ...
        # Process each strategy
        for strategy_name, signal_columns in strategy_signal_mapping.items():
