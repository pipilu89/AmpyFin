...
def create_position_column_vectorized(df):
    """
    Creates a 'position' column in the DataFrame based on the 'BBANDS_indicator' values.

    The position is set to 1 when 'BBANDS_indicator' changes from 'Hold' or 'Sell' to 'Buy'.
    It remains 1 until 'BBANDS_indicator' changes from 'Buy' to 'Sell' or from 'Hold' to 'Sell',
    at which point it reverts to the default value of 0.

    Parameters:
    -----------
    df : pandas.DataFrame
        DataFrame containing a 'BBANDS_indicator' column
    ...
    # Create a shifted version of BBANDS_indicator to detect changes
    df["prev_indicator"] = df["BBANDS_indicator"].shift(1)

    # Initialize position column with zeros
    df["position"] = 0

    # Define conditions for setting position to 1
    # Condition 1: BBANDS_indicator changes from Hold or Sell to Buy
    buy_signal = (df["prev_indicator"].isin(["Hold", "Sell"])) & (
        df["BBANDS_indicator"] == "Buy"
    )

    # Condition 2: BBANDS_indicator changes from Buy to Sell or from Hold to Sell
    sell_signal = (df["prev_indicator"].isin(["Buy", "Hold"])) & (
        df["BBANDS_indicator"] == "Sell"
    )
    ...
    df.drop("prev_indicator", axis=1, inplace=True)
...
if __name__ == "__main__":
    ...
    strategy_decisions_db_name = os.path.join(
        price_data_dir, "strategy_decisions_intermediate.db"
    )
    ...
    # ticker = "AAPL"
    ticker = "MSFT"
    df = sql_to_df_with_date_range(ticker, start_date, end_date, con_sd)
    cols_to_Keep = ["Open", "High", "Low", "Close", "Volume", "BBANDS_indicator"]
    df = df[cols_to_Keep]

    df = create_position_column_vectorized(df)
    df = create_cash_holdings_column(df)
    df = create_buy_and_sell_date_column(df)

    cols_to_Keep2 = ["buy_date", "buy_price", "sell_date", "sell_price"]
    df_trades = df[cols_to_Keep2][df["sell_date"] != ""]
    ...
    logger.info(f"{df_trades}")

    df.to_sql(ticker, con_tl, if_exists="replace")
